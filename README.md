...
| Level scripting | the trigger graph parses and executes, and a `CALL` now reaches its primitive: rotating brushes turn when a script asks, 46 requests moving 47 rotators across the disc. What a rotation call's operands mean differs per primitive and is read in one place, beside the builder. The gap this exposed is bigger than the fix: a **zone's** own script — 2,959 `CALL`s, 805 movers and 619 zone gates disc-wide — is never run by anything, while COMMON's is, and 886 of 886 trigger volumes point into COMMON |
