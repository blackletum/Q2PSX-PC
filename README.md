...
| Multiplayer | One shared arena supports one to four local players, with horizontal or vertical two-player splits and a quad layout for three or four. Each player has independent input, movement, pickups, damage, HUD, view weapon, camera and respawn state. Keyboard/mouse plus three SDL gamepads is the default; `--gamepad-player-one` allows four gamepads. Deathmatch, Team Deathmatch and Versus use the reconstructed QMULTI scoring and limits. Versus reloads rounds while preserving wins; the results screen waits for every player's fire press. The results text and readiness timing are retail-derived; its backdrop/layout and the return to the port's setup page are adapters. Extra pads currently use RIGHT STICK rather than independent controller-settings pages. The three cut flag/tag modes remain marked cut. |
...
For local play, the keyboard and mouse control player one; connected SDL gamepads
fill players two through four in connection order. Disconnecting a controller
releases its input without moving the other controllers to different players.
Use `--gamepad-player-one` to assign up to four gamepads starting with player one.
Additional players use the retail RIGHT STICK scheme: D-pad (or left stick) moves,
right stick looks, left shoulder fires, left trigger jumps, and right shoulder /
right trigger cycle weapons. South / Cross selects in menus, North / Triangle
goes back, and Start opens or closes the pause menu. The controller page selects
player one's analog scheme when a gamepad is assigned there.

Each player respawns with their own fresh fire press. Versus disables respawning,
preserves wins across arena reloads and ends at the round limit (`--dm-rounds N`
...
tests actual client glue, simulated SDL controllers, brief input taps at
60/144/240 Hz, shared pickups, damage, respawning, teleports, lighting ownership,
body retirement and results readiness without
opening a window or reading the keyboard/mouse.
